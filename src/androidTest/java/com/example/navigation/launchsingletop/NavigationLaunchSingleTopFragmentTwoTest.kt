...
class NavigationLaunchSingleTopFragmentTwoTest {

    @Test
    fun testNavigationLaunchSelf() {
        val navController = TestNavHostController(ApplicationProvider.getApplicationContext())

        val scenario = launchFragmentInContainer<NavigationLaunchSingleTopFragmentTwo>()
        scenario.onFragment {
            navController.setGraph(R.navigation.activity_navigation_launch_single_top)
            Navigation.setViewNavController(it.requireView(), navController)
            navController.navigate(R.id.navigationLaunchSingleTopFragmentTwo)
        }
